...
player : Float -> Player
player x = { x=x, y=0, vx=2, vy=0, score=0 }
player' x = { x=x, y=0, vx=-2, vy=0, score=0 }
...
stepPlyr t dir points player =
  let player1 = stepObj  t { player | vy <- (if player.vy /= 0 && dir == 1 then player.vy + 10 
                                             else if player.vy /= 0 && dir == -1 then player.vy - 10
                                             else dir * 400)  }
...
stepGame {space,dir1,dir2,delta} ({state,ball,player1,player2} as game) =
  let score1 = if ball.x >  halfWidth then 1 else 0
      score2 = if ball.x < -halfWidth then if player2.score > 9 then -1 else 1 else 0
